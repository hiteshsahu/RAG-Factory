...
# Convert jpg/jpeg/png images in a folder to JPG.
#
# Usage:
...
#
# Note: JPEG has no alpha channel -- a png with real transparency (e.g. a
# logo meant to sit on any background) gets flattened onto a white
# background here. Convert those by hand (e.g. to WebP) instead of via
# this script.
#
# Examples:
#   scripts/optimize-images.sh img
#   scripts/optimize-images.sh img/screen-shots -q 90 -r --delete-originals
...
if command -v magick >/dev/null 2>&1; then
  ...
  echo "Error: ImageMagick is not installed. Install it with 'brew install imagemagick'." >&2
  exit 1
fi
...
while IFS= read -r -d '' file; do
  ...
  if [[ "$file" == "$out" ]]; then
    echo "Skipping (already a .jpg): $file"
    continue
  fi
  ...
  "$MAGICK" "$file" -strip -background white -flatten -quality "$QUALITY" -sampling-factor 4:2:0 "$out"
